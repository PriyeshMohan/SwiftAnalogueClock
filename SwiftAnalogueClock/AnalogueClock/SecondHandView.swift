...
public class SecondHandView: ClockHandView {
    ...
    public override func addHandLayer() {
        ...
        let centralLayer = LayerUtil.arcLayer(arcCenter: CGPoint(x: bounds.width/2, y: 3*bounds.height/4), radius: bounds.width, startAngle: 0, endAngle: 2 * CGFloat.pi, strokeColor: strokeColor ?? .black, lineWidth: 1, fillColor: fillColor ?? .black, clockWise: true)
